...
public class ArmorUtils {
    ...
    public static class ModularHUD {
        ...
        private Pair<Integer, Integer> getStringCoord(int index) {
            ...
            switch (GAConfig.Misc.hudLocation) {
                case 1:
                    posX = 1 + GAConfig.Misc.hudOffsetX;
                    posY = 1 + GAConfig.Misc.hudOffsetY + (fontHeight * index);
                    break;
                case 2:
                    posX = windowWidth - (1 + GAConfig.Misc.hudOffsetX) - stringWidth;
                    posY = 1 + GAConfig.Misc.hudOffsetY + (fontHeight * index);
                    break;
                case 3:
                    posX = 1 + GAConfig.Misc.hudOffsetX;
                    posY = windowHeight - fontHeight * (stringAmount - index) - 1 - GAConfig.Misc.hudOffsetY;
                    break;
                case 4:
                    posX = windowWidth - (1 + GAConfig.Misc.hudOffsetX) - stringWidth;
                    posY = windowHeight - fontHeight * (stringAmount - index) - 1 - GAConfig.Misc.hudOffsetY;
